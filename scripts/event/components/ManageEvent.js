...
class ManageEvent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      names: this.props.event.people.map(name => ({ text: name, value: name })),
  ...
  />
    ...
    <Form error={!!this.props.error}>
      ...
      <Button
        ...
        basic
        color="black"
        content={formatMessage(messages.event.buttons.update)}
        icon="signup"
        labelPosition="left"
        onClick={this.props.updateEvent.bind(this, this.state.currentNames)}
      />
      <Button
        type="button"
        basic
        color="black"
        content={formatMessage(messages.event.buttons.return)}
        icon="reply"
        labelPosition="left"
